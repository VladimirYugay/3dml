...
class InferenceHandlerDeepSDF:
    ...
    def reconstruct(self, points, sdf, num_optimization_iters):
        ...
        loss_l1 = torch.nn.L1Loss().to(self.device)
        ...
        for iter_idx in range(num_optimization_iters):
            ...
            batch_indices = np.random.choice(len(points), self.num_samples)
            ...
            x = torch.cat([latent_codes, batch_points], -1)
            predicted_sdf = model(x)
            # TODO: truncate predicted sdf between -0.1 and 0.1
            predicted_sdf = torch.clamp(predicted_sdf, -0.1, 1)
    ...
    def interpolate(self, shape_0_id, shape_1_id, num_interpolation_steps):
        ...
        for i in range(0, num_interpolation_steps + 1):
            ...
            interpolated_code = latent_codes[0, :] * i + latent_codes[1, :] * (num_interpolation_steps - i)
